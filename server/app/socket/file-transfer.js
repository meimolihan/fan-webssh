...
// Basic transfer task creation
async function createTransferTask(transferConfig, socket, hostListDB, fileTransferDB, executeTransfer) {
  const { sourceHostId, targetHostId, sourcePaths, targetPath, method, options } = transferConfig
  if (!sourceHostId || !targetHostId || !sourcePaths || !targetPath) {
    throw new Error('Missing required transfer parameters')
  }

  const taskId = `transfer_${ Date.now() }_${ Math.random().toString(36).slice(2, 8) }`
  const task = {
    taskId,
    sourceHostId,
    targetHostId,
  ...
  await fileTransferDB.insertAsync(task)
  return task
}

// Task sorting function: running tasks first, then by updateTime descending
...
module.exports = (httpServer) => {
  ...
  serverIo.on('connection', async (socket) => {
    ...
    socket.on('start_transfer', async (transferConfig) => {
      try {
        const task = await createTransferTask(transferConfig, socket, hostListDB, fileTransferDB)
