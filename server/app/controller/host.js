...
async function batchUpdateHost({ res, request }) {
  const { hosts, updateData } = request.body
  if (!Array.isArray(hosts) || !updateData) return res.fail({ msg: '参数错误' })
  for (const id of hosts) {
    await hostListDB.updateAsync({ _id: id }, { $set: updateData })
  }
  res.success({ data: `批量更新成功,数量: ${ hosts.length }` })
}
